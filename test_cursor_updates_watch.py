...
class CursorUpdatesWatchTests(unittest.TestCase):
    ...
    def test_extract_x_graphql_operation(self) -> None:
        js_text = """
        333830:e=>{e.exports={queryId:"abc123",operationName:"UserByScreenName",operationType:"query",metadata:{featureSwitches:["flag_a","flag_b"],fieldToggles:["withPayments","withAuxiliaryUserLabels"]}}}
        """
        operation = watcher.extract_x_graphql_operation(js_text, "UserByScreenName")
        self.assertEqual(operation["query_id"], "abc123")
        self.assertEqual(operation["features"], ["flag_a", "flag_b"])
        self.assertEqual(
            operation["field_toggles"],
            ["withPayments", "withAuxiliaryUserLabels"],
        )

    def test_parse_x_graphql_timeline_expands_links_and_skips_retweets(self) -> None:
        payload = {
            "data": {
                "user": {
                    "result": {
        ...
        items = watcher.parse_x_graphql_timeline(payload)
        self.assertEqual(len(items), 1)
        self.assertEqual(items[0]["title"], "Hello https://cursor.com/docs")
        self.assertEqual(items[0]["summary"], "Hello https://cursor.com/docs")
        self.assertEqual(items[0]["url"], "https://x.com/cursor_ai/status/123")
    ...
    def test_select_items_with_seen_state_filters_old_history(self) -> None:
        now_utc = datetime(2026, 3, 9, 6, 0, tzinfo=timezone.utc)
        items = [
            {"id": "recent", "published_at": "2026-03-09T05:30:00Z"},
            {"id": "old-unseen", "published_at": "2026-03-01T12:00:00Z"},
        ]
        selected = watcher.select_items(
            items,
            ["already-seen"],
            "id",
            now_utc,
            last_checked_at="2026-03-09T05:00:00Z",
        )
        self.assertEqual([item["id"] for item in selected], ["recent"])

    def test_fetch_blog_items_with_seen_state_only_fetches_recent_unseen_articles(self) -> None:
        state = {
            "last_run_date": "2026-03-09",
            "last_checked_at": "2026-03-09T05:00:00Z",
            "seen": {
                "changelog": [],
                "blog": ["https://cursor.com/blog/already-seen"],
                "x": [],
            },
        }
        now_utc = datetime(2026, 3, 9, 6, 0, tzinfo=timezone.utc)
        ...
        original_fetch_text = watcher.fetch_text
        original_parse_blog_sitemap = watcher.parse_blog_sitemap
        original_parse_blog_article = watcher.parse_blog_article

        def fake_fetch_text(url: str) -> str:
            fetched_urls.append(url)
            return "<html></html>"

        def fake_parse_blog_sitemap(_xml_text: str) -> list[dict[str, str]]:
            return [
                {
                    "url": "https://cursor.com/blog/recent-unseen",
                    "lastmod": "2026-03-09T05:30:00Z",
                },
                {
                    "url": "https://cursor.com/blog/old-unseen",
                    "lastmod": "2026-03-01T12:00:00Z",
                },
                {
                    "url": "https://cursor.com/blog/already-seen",
                    "lastmod": "2026-03-09T05:45:00Z",
                },
            ]

        def fake_parse_blog_article(
            url: str,
            html_text: str,
            ...
            return {
                ...
                "url": url,
                "published_at": fallback_published_at or "2026-03-09T05:30:00Z",
                "summary": "",
            }

        watcher.fetch_text = fake_fetch_text
        watcher.parse_blog_sitemap = fake_parse_blog_sitemap
        watcher.parse_blog_article = fake_parse_blog_article
        try:
            items = watcher.fetch_blog_items(state, now_utc)
        finally:
            watcher.fetch_text = original_fetch_text
            watcher.parse_blog_sitemap = original_parse_blog_sitemap
            watcher.parse_blog_article = original_parse_blog_article

        self.assertEqual([item["url"] for item in items], ["https://cursor.com/blog/recent-unseen"])
        self.assertEqual(
            fetched_urls,
            [watcher.BLOG_SITEMAP_URL, "https://cursor.com/blog/recent-unseen"],
        )
