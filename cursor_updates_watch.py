...
def has_run_for_local_date(current_time: dt.datetime, previous_state: dict) -> bool:
    previous_local = previous_state.get("checked_at_local")
    if not previous_local:
        return False
    try:
        previous_time = dt.datetime.fromisoformat(previous_local)
    except ValueError:
        return False
    return previous_time.date() == current_time.date()


def should_run(current_time: dt.datetime, previous_state: dict | None = None) -> bool:
    if current_time.hour != 9:
        return False
    if previous_state and has_run_for_local_date(current_time, previous_state):
        return False
    return True
...
def write_outputs(snapshot: Snapshot, report_text: str) -> None:
    ...
    history_path = HISTORY_DIR / f"{snapshot.checked_at_local[:10]}.md"
...
def main() -> int:
    ...
    if not args.force and not should_run(current_local, previous_state):
        reason = "it is not 09:00 in Asia/Shanghai"
        if current_local.hour == 9 and has_run_for_local_date(current_local, previous_state):
            reason = "today's 09:00 Asia/Shanghai run already completed"
        print(
            f"Skipped Cursor update check because {reason}. "
