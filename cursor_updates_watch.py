...
def select_items(
    ...
    if seen_ids:
        ...
        return [item for item in items if item[id_key] not in seen_set]
...
def fetch_blog_items(state: dict[str, Any], now_utc: datetime) -> list[dict[str, Any]]:
    ...
    if seen_urls:
        candidates = [item for item in sitemap_items if item["url"] not in seen_urls]
    else:
        cutoff = now_utc - timedelta(days=BOOTSTRAP_DAYS)
        candidates = [
            item
...
def fetch_x_items() -> list[dict[str, Any]]:
    return parse_x_timeline_page(fetch_text(X_TIMELINE_URL))
...
def render_report(
    ...
    lines = [
        ...
        f"- Official X timeline: https://x.com/cursor_ai",
        "",
        (
...
def main(argv: list[str] | None = None) -> int:
    ...
    changelog_items = select_items(changelog_all, state["seen"]["changelog"], "id", now_utc)
    blog_items = select_items(blog_all, state["seen"]["blog"], "id", now_utc)
    x_items = select_items(x_all, state["seen"]["x"], "id", now_utc)
