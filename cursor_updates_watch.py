...
def fetch_x_timeline(
    ...
    """Try each configured public X mirror until one yields posts."""

    errors: list[str] = []
    for candidate_url in X_CANDIDATE_URLS:
        try:
            page_text = fetch_text(candidate_url)
            posts = parse_x_posts(page_text)
            if not posts:
                raise FetchError("mirror returned no visible posts")
            published_time = parse_x_published_time(page_text) or "unknown"
            return candidate_url, published_time, posts
...
def run(
    *,
    ...
    try:
        x_source_url, x_published_time, latest_x_posts = fetch_x_timeline(fetch_text)
        current_snapshot["x_posts"] = [hash_post(post) for post in latest_x_posts]
        if previous_x_hashes:
            new_x_posts = [
                post for post in latest_x_posts if hash_post(post) not in previous_x_hashes
            ]
        else:
            new_x_posts = latest_x_posts[:FIRST_RUN_X_LIMIT]
        source_successes += 1
    except FetchError as exc:
        warnings.append("official X fetch failed: " + str(exc))
