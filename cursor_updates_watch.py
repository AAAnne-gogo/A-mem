...
def parse_sitemap_entries(xml_text: str, section: str) -> list[dict[str, str]]:
    ...
    entries.sort(
        key=lambda item: (
            parse_iso_datetime(item["lastmod"]) if item["lastmod"] else datetime.min.replace(tzinfo=timezone.utc),
            item["url"],
        ),
        reverse=True,
    )
...
def build_report(
    *,
    ...
    new_changelog = compute_new_entries(previous_latest.get("changelog", []), changelog, "url")
    new_blog = compute_new_entries(previous_latest.get("blog", []), blog, "url")
    new_x_posts = compute_new_posts(previous_latest.get("x_posts", []), x_updates.get("posts", []))
